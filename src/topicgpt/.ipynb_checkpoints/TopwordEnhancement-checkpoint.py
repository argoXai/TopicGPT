...
class TopwordEnhancement:
    ...
    def generate_topic_name_str(self,
                            topwords: list[str],
                            n_words: int = None,
                            query_function: Callable = lambda tws: f"Please give me the common risk topic of those words: {tws}. Give me only the title of the topic and nothing else please. Make sure the title is precise and not longer than 5 words, ideally even shorter.") -> str:
        """
    ...
    def describe_topic_documents_sampling_completion_object(
        ...
        truncate_doc_thresh=100,
        n_documents: int = None,
        query_function: Callable = lambda docs: f"Please give me the common risk topic of the sample of those documents: {docs}. Note that the documents are truncated if they are too long. Also describe the various aspects and sub-topics of the topic.",
